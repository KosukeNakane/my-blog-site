...
export default function NewPostForm({ onCreated }: Props) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div>
        <label htmlFor="content" className="block text-sm font-medium mb-1">本文</label>
        <textarea
          id="content"
          value={content}
      ...
      <div>
        <label htmlFor="tags" className="block text-sm font-medium mb-1">タグ（カンマ or 空白区切り）</label>
        <input
          id="tags"
          type="text"
          value={tagsInput}
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={submitting} className="px-3 py-1 text-sm rounded-sm disabled:opacity-60" style={{ color: "#111", background: "linear-gradient(180deg, #f7f7f7 0%, #d9d9d9 100%)", border: "1px solid #6e6e6e", boxShadow: "0 1px 0 #ffffff inset, 0 -1px 0 #b4b4b4 inset" }}>
          {submitting ? "送信中..." : "保存"}
        </button>
